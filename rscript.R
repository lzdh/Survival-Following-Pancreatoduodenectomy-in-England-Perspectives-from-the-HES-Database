...
#plot OS
  ...
  plot(km, xlab = "Days", ylab = "Survival Probability", col = c("red", "blue", "chocolate", "lightpink","darkgreen"),
       lwd = 3, font = 2, cex.lab = 1.9, cex.axis = 2, xmax = 365, ymin = 0.6)
  legend("topright", c("Low volumn centre", "Low-med volumn centre", "Med-high volumn centre", "High volumn centre", "Vhigh volumn centre"), lty = 1,lwd= 4,
         col = c("red", "blue", "chocolate", "lightpink", "darkgreen"), bty = "n", text.font=2,cex=1.5)
  titleString = paste("Cancer related survival probability on centre volumns", 
                      " (pValue=", format(chiSquared, scientific=TRUE, digits=3), ")",sep="")
  title(titleString)
  
#plot 90day mortality  
  km <- survfit(Surv(data3$Survdiag, data3$`90_day_death` == 1) ~ data3$flag, data = data3)
  
  survivalDiff <- survdiff(Surv(data3$Survdiag, data3$`90_day_death` == 1) ~ data3$flag, data = data3, rho = 0) # rho = 0 gives no weight to any of the values
  chiSquared   <- pchisq(survivalDiff$chisq, 4, lower.tail=FALSE) # second entry is number of clusters-1
  
  par(mar=c(5,5,2,3))
  plot(km, xlab = "Days", ylab = "Survival Probability", col = c("red", "blue", "chocolate", "lightpink","darkgreen"),
       lwd = 3, font = 2, cex.lab = 1.9, cex.axis = 2, xmax = 90, ymin = 0.8)
  legend("bottomleft", c("Low volumn centre", "Low-med volumn centre", "Med-high volumn centre", "High volumn centre", "Vhigh volumn centre"), lty = 1,lwd= 4,
         col = c("red", "blue", "chocolate", "lightpink", "darkgreen"), bty = "n", text.font=2,cex=1.5)
  titleString = paste("90-day Survival probability on centre volumns", 
                      " (pValue=", format(chiSquared, scientific=TRUE, digits=3), ")",sep="")
  title(titleString)
...
library(survival)
cox_fit <- coxph(Surv(data3$Survdiag, data3$deathflag) ~ data3$centre_volume, data = data3, method="efron") 
summary(cox_fit)
...
model_cox <- coxph(Surv(data3$Survdiag, data3$deathflag) ~ sex+AgeAtdiag+eth_other
               +eth_asian+eth_black+eth_white+charlson_score+centre_volume+imd_quintile, data = data3, method = "breslow") 
