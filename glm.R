...
for(i in 1:3){
  ...
  model_glm <- glmnet(as.matrix(trainingData[, -which(names(Nset1) %in% c("surg_date","Survdiag","centre_flag",
                                                                          "X90daydeathflag","deathflag"))]), Surv(trainingData$Survdiag, trainingData$deathflag),
                      family="cox", maxit=1e4, alpha=1)
  
  cv.fit <- cv.glmnet(as.matrix(trainingData[, -which(names(Nset1) %in% c("surg_date","Survdiag","centre_flag",
                                                                          "X90daydeathflag","deathflag"))]),Surv(trainingData$Survdiag, trainingData$deathflag), 
                      family="cox", maxit=1e4, alpha=1)
  best_lambda <- cv.fit$lambda.min
  
  predictions <- predict(model_glm, newx = as.matrix(testData), s=best_lambda)
  
  for (j in 0:2666){
    data3$glm[3*j+i]<-predictions[j+1]
  }
  if(i==1) data3$glm[8002]<-predictions[2668]
  
}
#######forloop ends here

survConcordance(Surv(Survdiag, deathflag)~glm, data= data3)
